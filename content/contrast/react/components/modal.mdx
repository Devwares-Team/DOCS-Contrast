...
import { CDBModal, CDBCardImage, CDBCardBody, CDBCardTitle, CDBCardText, CDBCard, CDBBtn, CDBContainer } from "cdbreact-pro";
...
```jsx
import React, { useState } from "react";
import { CDBModal, CDBModalHeader, CDBModalBody, CDBModalFooter, CDBCardImage, CDBCardBody, CDBCardTitle, CDBCardText, CDBCard, CDBBtn, CDBContainer } from "cdbreact-pro";
  const [state, setState] = useState({
    modal1: false
  });
  const toggle = (nr) => () => {
