...
metaDescription: 'React Bootstrap 5 Select is a very useful component that the `↑` and `↓` arrow keys can be used to browse through options and the `↵` key to select the required option (works for stateful select).'
githubUrl: "https://github.com/Devwares-Team/DOCS-Contrast/blob/master/content/contrast/react/components/select.mdx"
...
React Bootstrap 5 Select is a very useful component that the `↑` and `↓` arrow keys can be used to browse through options and the `↵` key to select the required option (works for stateful select).

## Importing the React Bootstrap 5 Select Component

For you to use the React Bootstrap 5 Select components, you will need to import `CDBSelect` into your project.
